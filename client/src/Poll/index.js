...
class Poll extends Component {
  ...
  render(){
  	...
  	<a href={"https://twitter.com/intent/tweet?text=" + encodeURIComponent(text)}>
			<div className='short thin button normal-text poll-bottom'>Share</div>
		</a>
    let data = [
    	['Option', 'Votes']
    ]
    		...
    		<div className='container'>
	    		...
	    		<div id='poll-text'>
			    	...
			    	{this.state.poll.options.map( (option, i) => {
			    		return (
			    			<div className='center row' key={i} ><div id={`vote_${i}`}className='tall wide button poll-option' onClick={this.vote.bind(this)}>{option.name}</div></div>
			    		)
			    	})}
			    	<div className='center row'>
			    		{loggedIn ? button : null }
			    		{loggedIn ? <Link onClick={this.add.bind(this)}><div className='short thin button normal-text poll-bottom'>	Add Option</div></Link> : null}
			    	</div>
			    	<div className='center row'>
			    		{creator ? <Link onClick={this.delete.bind(this)}><div className='short thin button normal-text poll-bottom'>Delete</div></Link> : null }
			    	</div>
